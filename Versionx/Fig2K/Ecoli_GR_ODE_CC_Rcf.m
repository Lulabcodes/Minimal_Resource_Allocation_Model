function [dxdt,growthRate,kelong,fracCharge,fracRactive] = Ecoli_GR_ODE_CC_Rcf(t,x,nutr,cm,hp,ppGpp,model)
...
switch model
    case 'hill2'
        JsR = kelongTot*hp.phiRMax*(hp.thetaPpGppR^2./(hp.thetaPpGppR^2+ppGpp^2))/hp.massR;
    case 'linear'
        JsR = kelongTot*hp.phiRMax*(1-ppGpp/300)/hp.massR;
    case 'exponential'
        JsR = kelongTot*hp.phiRMax*(exp(-ppGpp/hp.thetaPpGppR))/hp.massR;
    case 'parabolic'
        JsR = kelongTot*hp.phiRMax*(1-(ppGpp/300)^2)/hp.massR;
    otherwise
